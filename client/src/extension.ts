...
export function activate(context: vscode.ExtensionContext) {
    console.log('on activate, your extension "mvel"....');
    ...
    if (DEBUG_MODE === 'true') {
        ...
        let serverJar = path.join(__dirname, "..", 'lib', 'mvel3-lsp-server-jar-with-dependencies.jar');
    ...
    if (serverOptions) {
        ...
        let clientOptions: LanguageClientOptions = {
            // Register the server for mvel3 documents
            documentSelector: [{scheme: 'file', language: 'mvel3'}]
        };
...
export function deactivate() { 
	console.log('Your extension "mvel" is now deactivated!');
}
